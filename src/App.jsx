...
const App = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.message && err.message.includes('429')) {
        setError(
          'AI Rate Limit Exceeded ⏳: You have requested too many translations per minute on the Google Free Tier. Please wait exactly 60 seconds and click Generate again!'
        )
      } else {
  ...
  return (
    <div
      className={`min-h-screen pb-20 transition-colors duration-500 relative overflow-hidden ${riskLevel === 'critical' ? 'bg-red-50' : 'bg-slate-50'}`}
    >
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only p-4 bg-blue-600 text-white fixed top-0 left-0 z-50 rounded-br-xl shadow-lg"
      >
      ...
      <main id="main-content" className="container max-w-4xl pt-12 relative z-10">
        <section className="mb-12 text-center">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-1-5 rounded-full bg-blue-50 text-blue-700 font-bold text-xs mb-6 shadow-sm border border-blue-500-10"
          >
          ...
          <motion.h2
            ...
            className="text-4xl md:text-5xl lg:text-6xl font-extrabold mb-4 text-slate-900"
          >
            Intelligent Triage.
            <br />
          ...
          <motion.p className="text-slate-600 max-w-xl mx-auto font-medium text-lg">
            Transform chaotic voice notes, medical photos, and messy text into structured,
            life-saving operational data in milliseconds.
          </motion.p>
        </section>

        <section
          className={`glass-panel p-8 mb-8 transition-all duration-500 ${riskLevel === 'critical' ? 'critical-pulse' : ''} relative`}
        >
          <form onSubmit={handleSubmit}>
            <div className="relative mb-6">
              ...
              >
                ...
                <span className="group relative cursor-pointer text-blue-500">
                  ...
                  <span className="tooltip-text font-normal">
                    Type via keyboard or click the microphone to dictate hands-free. Attach photos
                    of prescriptions or scenes!
                  </span>
                </span>
              </label>
              ...
              <div className="relative">
                ...
                {isProcessing && (
                  <div className="absolute inset-0 bg-white-80 backdrop-blur-md rounded-2xl flex flex-col items-center justify-center z-10 border-2 border-blue-400">
                    ...
                    <h3 className="text-xl font-extrabold text-blue-600 tracking-widest uppercase">
                      Synthesizing Intelligence
                    </h3>
                    <p className="text-sm font-bold mt-2 text-slate-500 animate-pulse">
                      Running Multimodal Medical AI Models...
                    </p>
                  </div>
                )}
                ...
                <div className="absolute bottom-6 right-6 flex items-center gap-3">
                  <div className="group relative">
                    <button
                      type="button"
                      onClick={startSpeech}
                      className={`p-3 w-12 h-12 flex justify-center items-center rounded-full transition-all shadow-md ${isListening ? 'bg-red-500 text-white animate-pulse shadow-xl' : 'bg-white border border-slate-200 text-slate-700 hover-scale-105'}`}
                    >
                      {isListening ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
                    </button>
                    <span className="tooltip-text">
                      {isListening ? 'Listening... tap to stop' : 'Start Voice Dictation'}
                    </span>
                  </div>
                  <div className="group relative">
                    <button
                      ...
                      className="p-3 w-12 h-12 flex justify-center items-center rounded-full bg-white border border-slate-200 text-slate-700 hover-scale-105 transition-all shadow-md"
                    >
            ...
            {images.length > 0 && (
              <div className="flex flex-wrap gap-4 mb-6 p-4 bg-slate-50 border border-slate-200 rounded-xl shadow-sm">
                <p className="w-full text-xs font-bold text-slate-500 uppercase">
                ...
                {images.map((img, idx) => (
                  <div key={idx} className="relative group">
                    <img
                      src={img}
                      className="w-20 h-20 object-cover rounded-lg shadow-sm border border-slate-200 hover-scale-105 transition-all"
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center justify-between pt-4 border-t border-slate-200 mt-6">
              <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                <div className="group relative w-full sm:w-auto">
                  <button
                    ...
                    disabled={isLocating}
                    className={`flex items-center justify-center w-full gap-2 px-6 py-3 rounded-xl border shadow-sm transition-all font-semibold ${location && location.toString().includes('Lat') ? 'bg-teal-50 border-teal-200 text-teal-700' : 'bg-white border-slate-200 text-slate-700 hover-scale-102'}`}
                  >
                    <MapPin
                      className={`w-5 h-5 ${isLocating ? 'animate-pulse text-blue-500' : location && location.toString().includes('Lat') ? 'text-teal-600' : 'text-slate-400'}`}
                    />
                    {location && location.toString().includes('Lat') ? 'Signal Locked' : 'Auto GPS'}
                  </button>
                  <span className="tooltip-text">
                    Detect exact geographical coordinates via browser
                  </span>
                </div>
                ...
                <div className="w-full sm:w-64 relative">
                  ...
                  {location && location.toString().includes('Lat') && (
                    <a
                      ...
                      className="absolute -bottom-5 left-2 text-[10px] text-blue-600 font-bold uppercase transition-all hover-underline"
                    >
                      Verify Map Routing →
                    </a>
                  )}
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    GENERATE INTELLIGENCE <Send className="w-5 h-5" />
                  </>
                )}
              </button>
            </div>
            {error && (
              <p className="mt-4 text-red-600 font-bold bg-red-50 p-4 rounded-xl border border-red-200 shadow-sm flex items-start gap-2">
          ...
          {result && (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <div
                  className={`p-3 rounded-xl shadow-md ${riskLevel === 'critical' ? 'bg-red-100 text-red-600 border border-red-200' : 'bg-teal-50 text-teal-600 border border-teal-200'}`}
                >
                  {riskLevel === 'critical' ? (
                ...
                {riskLevel === 'critical' && (
                  <div className="p-8 rounded-2xl bg-white border-2 border-red-500 flex flex-col md:flex-row items-center md:items-start gap-6 shadow-xl shadow-red-500-20">
                    ...
                    <div className="flex-1 text-center md:text-left">
                      <h4 className="text-red-700 font-extrabold text-2xl mb-2 tracking-tight">
                        CRITICAL INCIDENT RESPONSE REQUIRED
                      </h4>
                      <p className="text-slate-600 text-base font-medium mb-6">
                        Automated emergency protocols triggered. Please maintain situational
                        awareness and ensure safe perimeter for priority responders.
                      </p>

                      <div className="flex flex-col sm:flex-row gap-4">
                        <button
                          onClick={() => (window.location.href = 'tel:108')}
                          className="px-8 py-3 bg-red-600 rounded-xl font-extrabold text-white hover-bg-red-500 transition-colors shadow-lg shadow-red-500-30 flex items-center justify-center gap-2 text-lg hover-scale-105 active-scale-95"
                        >
